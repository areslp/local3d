...
if (USE_FFTW)
    ...
    libs = sprintf('%s -L"%s" %s', libs, FFTW_DIR_LIB, FFTW_NAME);
end

if (USE_SWHT)
    incs = sprintf('%s -I"%s"', incs, SWHT_DIR_INC);
    % libs = sprintf('%s -L"%s" -l%s', libs, SWHT_DIR_LIB, SWHT_NAME);
    libs = sprintf('%s -L"%s" %s', libs, SWHT_DIR_LIB, SWHT_NAME);
end

incs
libs

prefix0 = sprintf('mex -v -outdir ''%s'' %s -O %s', MEX_DIR, incs, libs); 
